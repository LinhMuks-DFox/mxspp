...
def setup_venv():
    ...
    try:
        ...
        if pathlib.Path(REQUIREMENTS_FILE).is_file():
            ...
            subprocess.run(
                [str(pip_executable), "install", "-r", REQUIREMENTS_FILE], check=True
            )
...
def check_system_dependencies():
    ...
    for cmd in ["clang", "clang++"]:
        if not shutil.which(cmd):
            error(
                f"Compiler '{cmd}' not found. Please install clang suite (version >= 20)."
            )
            all_ok = False
            continue
        try:
            ...
            if match and int(match.group(1)) < 20:
                error(
                    f"Found {cmd} version {match.group(1)}, but version >= 20 is required."
                )
                all_ok = False
            elif match:
                ...
                warn(
                    f"Could not determine version for {cmd}. Please verify it is >= 20."
                )
        except (subprocess.CalledProcessError, FileNotFoundError) as e:
    ...
    if sys.platform == "linux":
        try:
            ...
            warn(
                "`ldconfig` command not found or failed. Cannot check for libc++. Please verify it is installed."
            )

    if not all_ok:
        error(
            "System dependency checks failed. Please resolve the issues mentioned above."
        )
        sys.exit(1)
    ...
    try:
        ...
        if archive_path.suffix == ".zip":
            ...
            raise NotImplementedError(
                f"Extraction for {archive_path.suffix} not supported."
            )
...
def setup_prebuilt_llvm():
    ...
    if sys.platform not in LLVM_CONFIG["prebuilt"]:
        error(
            f"No pre-built LLVM available for your OS: {sys.platform}. Please use --build-from-source."
        )
        sys.exit(1)
...
def build_llvm_from_source():
    ...
    if not all(shutil.which(cmd) for cmd in ["cmake", "ninja"]):
        error(
            "Build requires `cmake` and `ninja`. Please install them and ensure they are in your PATH."
        )
        sys.exit(1)
    ...
    try:
        ...
        subprocess.run(
            ["cmake", "--build", str(build_dir), "--target", "install"], check=True
        )
        ...
        error(
            f"Build directory '{build_dir}' is kept for inspection and incremental builds."
        )
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(
        description="Download and set up project dependencies."
    )
    parser.add_argument(
        "-b",
        "--build-from-source",
