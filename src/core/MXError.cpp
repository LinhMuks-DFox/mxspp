...
namespace mxs::core {
    MXError::MXError(error_type_name_t error_type, message_t message,
                     MXObjectOwned alternative, bool panic, bool is_static)
        : MXObject(is_static),
    ...
    auto MXError::repr() const -> repr_t {
        return std::format("{}(panic={}): {}", this->error_type_, this->panic_,
                           this->message_);
    }
}