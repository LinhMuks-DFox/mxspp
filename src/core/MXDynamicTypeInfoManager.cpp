...
namespace mxs::core {
    ...
    auto MXDynamicTypeInfoManager::get_rtti() -> MXRuntimeTypeInfo & {
        static MXRuntimeTypeInfo rtti{
            "mxs::core::MXDynamicTypeInfoManager",
            &MXObject::get_rtti(),
        };

        return rtti;
    }
    auto MXDynamicTypeInfoManager::register_newtype(const MXRuntimeTypeInfo *const obj)
            -> void {
        std::scoped_lock guard(this->lock);
        this->type_infos.insert(obj);
    }

    auto MXDynamicTypeInfoManager::unregister_type(const MXRuntimeTypeInfo *const obj)
            -> void {
        std::scoped_lock guard(this->lock);
